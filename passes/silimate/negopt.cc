	...
	void execute(std::vector<std::string> args, RTLIL::Design *design) override
	{
		...
		for (auto module : design->selected_modules()) {
			if (run_pre) {
				...
				// pre-subpass creates the patterns they match.  Separate pm
				// instances so sub2neg sees the $sub cells manual2sub creates.
				{
				...
				// Fixed-point: negexpand/negneg/negmux can feed each other.
				did_something = true;
