...
void merge_operators(RTLIL::Module *module, RTLIL::Cell *mux, const std::vector<OpMuxConn> &ports, const ExtSigSpec &operand, const SigMap &sigmap)
{
	...
	if (GetSize(shared_pmux_s) == 1) {
		mux_to_oper = module->Mux(NEW_ID2_SUFFIX("mux"), shared_pmux_a, shared_pmux_b, shared_pmux_s, cell->get_src_attribute(), cell->get_submod_attribute()); // SILIMATE: Improve the naming
	} else {
		mux_to_oper = module->Pmux(NEW_ID2_SUFFIX("pmux"), shared_pmux_a, shared_pmux_b, shared_pmux_s, cell->get_src_attribute(), cell->get_submod_attribute()); // SILIMATE: Improve the naming
	}
